    ...
    if (offset < 0x70) {
        ...
        switch (offset) {
            ...
            case 0x74: {
                ...
                log_set_quiet(0);
                log_fatal("DICR write force=%u ie=%02x irqen=%u clear=%02x -> force=%u ie=%02x irqen=%u clear=%02x",
                    (old_value & DICR_FORCE) != 0,
                    (old_value & DICR_FLGEN) >> 16,
                    (old_value & DICR_IRQEN) != 0,
                    (old_value & DICR_FLAGS) >> 24,
                    (dma->dicr & DICR_FORCE) != 0,
                    (dma->dicr & DICR_FLGEN) >> 16,
                    (dma->dicr & DICR_IRQEN) != 0,
                    (dma->dicr & DICR_FLAGS) >> 24
                );
                log_set_quiet(1);
            } break;
...
void psx_dma_do_mdec_in(psx_dma_t* dma) {
    ...
    log_fatal("MDEC_IN DMA transfer: madr=%08x, dir=%s, sync=%s (%u), step=%s, size=%x",
        dma->mdec_in.madr,
        CHCR_TDIR(mdec_in) ? "to device" : "to RAM",
        g_psx_dma_sync_type_name_table[CHCR_SYNC(mdec_in)], CHCR_SYNC(mdec_in),
        CHCR_STEP(mdec_in) ? "decrementing" : "incrementing",
        BCR_SIZE(mdec_in)
    );
...
void psx_dma_do_mdec_out(psx_dma_t* dma) {
    ...
    log_fatal("MDEC_OUT DMA transfer: madr=%08x, dir=%s, sync=%s (%u), step=%s, size=%x",
        dma->mdec_out.madr,
        CHCR_TDIR(mdec_out) ? "to device" : "to RAM",
        g_psx_dma_sync_type_name_table[CHCR_SYNC(mdec_out)], CHCR_SYNC(mdec_out),
        CHCR_STEP(mdec_out) ? "decrementing" : "incrementing",
        BCR_SIZE(mdec_out)
    );
    ...
    if (dma->spu_irq_delay) {
        ...
        if (dma->spu_irq_delay <= 0)
            if (dma->dicr & DICR_DMA4EN) {
                ...
                log_set_quiet(0);
                log_fatal("set SPU DMA IRQ flag dicr=%08x", dma->dicr);
                log_set_quiet(1);
            }
    }
