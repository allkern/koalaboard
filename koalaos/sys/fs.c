...
static fstatus fat_follow_path(struct dir_s* dir, const char* path, u32 length) {
    ...
    while (1) {
        ...
        // Check if the next fragment exists
        if (*path++ == '\0') break;
        if (*path == '\0') break;
...
void fat_print_info(struct info_s* info) {
    if (info->attribute & ATTR_VOL_LABEL)
        return;
    ...
    if (info->attribute & ATTR_DIR) {
        ...
        printf("     %9d%c ", size, ext ? ext : 'B');
    }

    for (int i = 0; i < info->name_length; i++)
        putchar(info->name[i]);
...
void fat32_thread(void* arg) {
    ...
    // printf(BLUE "Displaying system volumes:\n");
...
u8 disk_mount(disk_e disk) {
    ...
    if (!has_partitions) {
        printf("Unable to mount (No partitions on disk)\n");
    ...
    for (u8 i = 0; i < 4; i++) {
        if (partitions[i].lba) {
            ...
            if (fat_search(mount_buffer)) {
                ...
                struct volume_s* vol = malloc(sizeof(struct volume_s));
                ...
                vol->serial = fat_load32(mount_buffer + BPB_32_VOL_ID);
...
/// Move the read / write file pointer. The offset is computed with respect 
